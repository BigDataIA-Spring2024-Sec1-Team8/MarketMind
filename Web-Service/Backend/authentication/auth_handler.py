    ...
    try:
        with SessionLocal() as session:
            existing_user = session.execute(
                "SELECT * FROM MARKETPLACE.PUBLIC.USERS WHERE USERNAME = :username",
            ...
            session.execute(
                "INSERT INTO MARKETPLACE.PUBLIC.USERS (USERNAME, PASSWORD, GENDER) VALUES (:username, :password, :gender)",
                {
                    "username": user.username,
            ...
            user_id = session.execute(
                "SELECT ID FROM MARKETPLACE.PUBLIC.USERS WHERE USERNAME = :username",
                {"username": user.username},
            ).fetchone()[0]
            ...
            for activity in user.activities:
                session.execute(
                    "INSERT INTO MARKETPLACE.PUBLIC.USERSACTIVITIES (USER_ID, ACTIVITY) VALUES (:user_id, :activity)",
                    {
                        "user_id": user_id,
                        "activity": activity,
                    },
